...
class PrerequisiteService {
	...
	async getPrereqs(courseId) {
		...
		const r = await db.query(
			'SELECT c.course_id, c.course_code, c.title FROM prerequisites p INNER JOIN courses c ON p.prerequisite_course_id = c.course_id WHERE p.course_id = ? ORDER BY c.course_code ASC',
			[courseId]
		);
