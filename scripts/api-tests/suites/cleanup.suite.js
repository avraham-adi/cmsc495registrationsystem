...
export async function runCleanupSuite(env) {
    ...
    await harness.run(
        'Admins can remove enrollment records',
        `DELETE /enrollment/${ctx.enrollment1?.enrollment_id ?? ':id'}`,
    ...
    await harness.run(
        'Primary course can also be deleted after cleanup',
        `DELETE /course/${ctx.course1?.course_id ?? ':id'}`,
        { method: 'DELETE', client: 'admin' },
        { status: 200 },
        async () => {
    ...
    await harness.run(
        'Admins can delete temporary users during cleanup',
        `DELETE /admin/${ctx.users.temp.user?.id ?? ':id'}`,
        { method: 'DELETE', client: 'admin' },
        { status: 200 },
        async () => {
    ...
    if (ctx.users.boundaryUser?.id) {
        await harness.run(
            'Boundary-test users can be deleted during cleanup',
            `DELETE /admin/${ctx.users.boundaryUser.id}`,
            { method: 'DELETE', client: 'admin' },
            { status: 200 },
            async () => {
    ...
    await harness.run(
        'Admin logout clears the active session',
        'GET /user/logout',
        ...
        async () => {
            ...
            assertStatus(response, 200);
            assert(
                typeof response.headers['set-cookie'] === 'string' && response.headers['set-cookie'].includes('sid='),
                'Admin logout should emit a clearing Set-Cookie header.'
            );
            admin.clear();
            return harness.responseSummary(response);
        }
    ...
    await harness.run(
        'Student can still log out after the protected workflow completes',
        'GET /user/logout',
        ...
        async () => {
            ...
            assert(
                typeof response.headers['set-cookie'] === 'string' && response.headers['set-cookie'].includes('sid='),
                'Student logout should emit a clearing Set-Cookie header.'
            );
            student.clear();
            return harness.responseSummary(response);
        }
    ...
    await harness.run(
        'Logged-out admin sessions cannot access /user/me',
        'GET /user/me',
        { method: 'GET', client: 'admin' },
        { status: 401 },
        async () => {
