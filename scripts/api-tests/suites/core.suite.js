...
export async function runCoreSuite(env) {
	...
	await harness.run('Health check returns service status', 'GET /api/health', { method: 'GET' }, { status: 200, body: { message: 'API is running' } }, async () => {
	...
	await harness.run('Unknown route returns 404', 'GET /missing-route', { method: 'GET' }, { status: 404, body: { error: 'Route Not Found' } }, async () => {
	...
	await harness.run('Concurrent health checks succeed', 'GET /api/health', { method: 'GET', concurrency: 8 }, { status: 200, allResponses: true }, async () => {
		...
		assert(
			...
			'At least one concurrent health request failed.'
		);
	...
	await harness.run('Logout requires an authenticated session', 'GET /user/logout', { method: 'GET' }, { status: 401 }, async () => {
	...
	await harness.run('Authenticated admin can read /user/me', 'GET /user/me', { method: 'GET', client: 'admin' }, { status: 200, bodyKeys: ['User'] }, async () => {
	...
	await harness.run('Concurrent authenticated admin requests succeed', 'GET /user/me', { method: 'GET', client: 'admin', concurrency: 6 }, { status: 200, allResponses: true }, async () => {
		const responses = await Promise.all(Array.from({ length: 6 }, () => admin.request('/user/me')));
		assert(
			responses.every((response) => response.status === 200),
			'At least one concurrent /user/me request failed.'
		);
		return responses.map(harness.responseSummary);
	});
	...
	await harness.run('Admin list rejects invalid role filters', 'GET /admin?role=INVALID', { method: 'GET', client: 'admin', query: { role: 'INVALID' } }, { status: 400 }, async () => {
