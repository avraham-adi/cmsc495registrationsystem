...
export async function runUnitStyleSuite(env) {
    ...
    await harness.run(
        'User domain helpers preserve role and session metadata',
        'UNIT User.fromPersistence',
        { type: 'unit' },
        { pass: true },
        async () => {
    ...
    await harness.run(
        'Section domain normalizes async schedule values',
        'UNIT Section.fromPersistence',
        { type: 'unit' },
        { pass: true },
        async () => {
    ...
    await harness.run(
        'Session payload generation only stores minimal auth state',
        'UNIT SessionService.createPld',
        { type: 'unit' },
        { pass: true },
        async () => {
    ...
    await harness.run(
        'Last-admin protections remain enforced at the service layer',
        'UNIT AdminService last-admin guard',
        { type: 'service' },
        { pass: true },
        async () => {
    ...
    await harness.run(
        'EnrollmentService enforces the waitlist-full boundary',
        'UNIT EnrollmentService.addEnroll',
        { type: 'service' },
        { pass: true },
        async () => {
    ...
    await harness.run(
        'SectionService.getHighIdx keeps code numbering continuity',
        'UNIT SectionService.getHighIdx',
        { type: 'unit' },
        { pass: true },
        async () => {
    ...
    await harness.run(
        'SectionService.revAcCodes removes both code and used-flag pairs',
        'UNIT SectionService.revAcCodes',
        { type: 'service' },
        { pass: true },
        async () => {
            const service = new SectionService();
    ...
    await harness.run(
        'SectionService.getCtx rejects professors accessing sections they do not own',
        'UNIT SectionService.getCtx',
        { type: 'service' },
        { pass: true },
        async () => {
            const service = new SectionService();
    ...
    await harness.run(
        'EnrollmentService.enrollWithCode rejects invalid or already-used access codes',
        'UNIT EnrollmentService.enrollWithCode',
        { type: 'service' },
        { pass: true },
        async () => {
            const service = new EnrollmentService();
    ...
    await harness.run(
        'AuthService rejects duplicate emails during profile updates',
        'UNIT AuthService.updUser',
        { type: 'service' },
        { pass: true },
        async () => {
    ...
    await harness.run(
        'AdminService.setRole leaves exactly one role-table row after repeated transitions',
        'UNIT AdminService.setRole',
        { type: 'service' },
        { pass: true },
        async () => {
            const service = new AdminService();
    ...
    await harness.run(
        'SessionService.destroy clears the cookie and tears down the session',
        'UNIT SessionService.destroy',
        { type: 'unit' },
        { pass: true },
        async () => {
    ...
    await harness.run(
        'Domain constructors reject malformed empty persistence rows',
        'UNIT domain fromPersistence',
        { type: 'unit' },
        { pass: true },
        async () => {
            ...
            try { User.fromPersistence(null); } catch { userFailed = true; }
            try { Section.fromPersistence(null); } catch { sectionFailed = true; }
            try { Enrollment.fromPersistence(null); } catch { enrollmentFailed = true; }
