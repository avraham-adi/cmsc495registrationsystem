...
export async function runConcurrencySuite(env) {
    ...
    ctx.concurrency = {
        users: {
            a: { name: 'Runner Race Student A', email: 'runner.race.a@example.edu', detail: 'Computer Science', type: 'STUDENT', password: null, nextPassword: strongPassword('RaceStudentA'), user: null, client: new clients.student.constructor(harness.baseUrl) },
            b: { name: 'Runner Race Student B', email: 'runner.race.b@example.edu', detail: 'Computer Science', type: 'STUDENT', password: null, nextPassword: strongPassword('RaceStudentB'), user: null, client: new clients.student.constructor(harness.baseUrl) },
            c: { name: 'Runner Race Student C', email: 'runner.race.c@example.edu', detail: 'Computer Science', type: 'STUDENT', password: null, nextPassword: strongPassword('RaceStudentC'), user: null, client: new clients.student.constructor(harness.baseUrl) },
            d: { name: 'Runner Race Student D', email: 'runner.race.d@example.edu', detail: 'Computer Science', type: 'STUDENT', password: null, nextPassword: strongPassword('RaceStudentD'), user: null, client: new clients.student.constructor(harness.baseUrl) },
        },
        course: null,
        raceSection: null,
        raceCodes: null,
    ...
    await harness.run(
        ...
        async () => {
            ...
            assert(responses.every((response) => response.status === 201), 'Each concurrent enrollment attempt should produce an enrollment record.');
    ...
    await harness.run(
        ...
        async () => {
            ...
            assert(
                ['Runner Student Concurrency A', 'Runner Student Concurrency B'].includes(final.body?.User?.name),
                'Final profile name should match one of the concurrent successful updates.'
            );
            return {
    ...
    await harness.run(
        ...
        async () => {
            ...
            const results = await Promise.allSettled([
                service.updEnroll(targetA.enrollment_id, 'dropped', actingUser, null),
                service.rmvEnroll(targetB.enrollment_id, actingUser),
            ]);

            assert(results.every((result) => result.status === 'fulfilled'), 'Drop/delete race operations should settle successfully.');
