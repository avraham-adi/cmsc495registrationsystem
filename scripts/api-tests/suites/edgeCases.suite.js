...
export async function runEdgeCasesSuite(env) {
    ...
    await harness.run(
        'Pagination rejects page zero',
        'GET /course?page=0',
        { method: 'GET', query: { page: 0 } },
        { status: 400 },
        async () => {
    ...
    await harness.run(
        'Pagination rejects limits above 100',
        'GET /admin?limit=101',
        { method: 'GET', client: 'admin', query: { limit: 101 } },
        { status: 400 },
        async () => {
    ...
    await harness.run(
        'Pagination accepts the upper limit boundary of 100',
        'GET /course?page=1&limit=100',
    ...
    await harness.run(
        'Pagination accepts the minimum positive page and limit boundaries',
        'GET /course?page=1&limit=1',
    ...
    await harness.run(
        'Semester creation rejects years below the minimum',
        'POST /semester',
        { method: 'POST', client: 'admin', body: { term: 'Spring', year: 1899 } },
        { status: 400 },
        async () => {
    ...
    await harness.run(
        'Deleting a non-existent prerequisite pair returns 404',
        'DELETE /prerequisite/999998/999999',
    ...
    await harness.run(
        'Prerequisite reads can return an empty list',
        `GET /prerequisite/${ctx.course2?.course_id ?? ':id'}`,
        { method: 'GET' },
    ...
    await harness.run(
        'Multi-hop prerequisite cycles are rejected',
        'POST /course + POST /prerequisite',
    ...
    await harness.run(
        'Semester creation accepts the maximum year boundary',
        'POST /semester',
        { method: 'POST', client: 'admin', body: { term: 'Spring', year: 3000 } },
        { status: 201 },
        async () => {
            const response = await admin.request('/semester', {
                method: 'POST',
    ...
    await harness.run(
        'Deleting a non-existent semester returns 404',
        'DELETE /semester/999999',
        { method: 'DELETE', client: 'admin' },
        { status: 404 },
        async () => {
    ...
    await harness.run(
        'Semester list is ordered by year descending then id descending',
        'GET /semester',
        { method: 'GET' },
        { status: 200, ordered: true },
        async () => {
            ...
            for (let i = 1; i < rows.length; i++) {
                ...
                const cur = rows[i];
                assert(
                    prev.year > cur.year || (prev.year === cur.year && prev.semester_id >= cur.semester_id),
                    'Semester list ordering should be descending by year and then semester_id.'
                );
            }
            return harness.responseSummary(response);
        }
    ...
    await harness.run(
        'User profile update allows unchanged payloads',
        'PUT /user/me',
        { method: 'PUT', client: 'student', body: '[unchanged profile]' },
        { status: 200 },
        async () => {
    ...
    await harness.run(
        'Profile updates do not change the current password for fresh logins',
        'POST /user/login',
        { method: 'POST', client: 'student-fresh-login' },
        { status: 200 },
        async () => {
    ...
    await harness.run(
        ...
        async () => {
            ...
            assert(
                validation.body?.details && typeof validation.body.details === 'object',
                '400 validation responses should include structured details.'
            );
            assert(
                validation.body?.details?.fieldErrors && typeof validation.body.details.fieldErrors === 'object',
                '400 validation responses should include fieldErrors.'
            );
    ...
    await harness.run(
        ...
        async () => {
            ...
            assert(responses.every((response) => response.status === 401), 'Every stale-session request should be rejected.');
    ...
    await harness.run(
        ...
        `DELETE /admin/${ctx.admin.user?.id ?? ':id'}`,
        { method: 'DELETE', client: 'admin' },
        { status: 403 },
        async () => {
    ...
    await harness.run(
        ...
        async () => {
            ...
            const revokedCode = Object.entries(codesResponse.body).find(
                ([key, value]) => /^code\d+$/.test(key) && typeof value === 'string' && value !== ctx.concurrency.promotedCode
            )?.[1];
            assert(revokedCode, 'Revoked code is missing.');

            const revokeResponse = await professor.request(
                `/section/${ctx.concurrency.waitSection.section_id}/access-codes?codes=${encodeURIComponent(revokedCode)}`,
                { method: 'DELETE' }
            );
    ...
    await harness.run(
        ...
        async () => {
            ...
            assert(responses.slice(0, 4).every((response) => response.status === 201), 'The first four waitlist-full setup enrollments should create records.');
    ...
    await harness.run(
        ...
        { status: 400 },
        async () => {
            const target = ctx.concurrency.waitEnrollments.d?.status === 'waitlisted'
                ? ctx.concurrency.waitEnrollments.d.enrollment_id
                : ctx.concurrency.waitEnrollments.c.enrollment_id;
    ...
    await harness.run(
        'Deleting a non-existent enrollment returns 404',
        'DELETE /enrollment/999999',
        { method: 'DELETE', client: 'admin' },
        { status: 404 },
        async () => {
    ...
    await harness.run(
        'Not-found course reads return 404',
        'GET /course/999999',
        { method: 'GET' },
        { status: 404 },
        async () => {
    ...
    await harness.run(
        'Not-found semester reads return 404',
        'GET /semester/999999',
        { method: 'GET' },
        { status: 404 },
        async () => {
    ...
    await harness.run(
        'Not-found section reads return 404',
        'GET /section/999999',
        { method: 'GET' },
        { status: 404 },
        async () => {
    ...
    await harness.run(
        'Not-found section deletes return 404',
        'DELETE /section/999999',
        { method: 'DELETE', client: 'admin' },
        { status: 404 },
        async () => {
    ...
    await harness.run(
        'Not-found prerequisite reads return 404',
        'GET /prerequisite/999999',
        { method: 'GET' },
        { status: 404 },
        async () => {
    ...
    await harness.run(
        'Not-found admin reads return 404',
        'GET /admin/999999',
    ...
    await harness.run(
        'Not-found admin deletes return 404',
        'DELETE /admin/999999',
        { method: 'DELETE', client: 'admin' },
        { status: 404 },
        async () => {
    ...
    await harness.run(
        'Not-found enrollment reads return 404',
        'GET /enrollment/999999',
        { method: 'GET', client: 'admin' },
        { status: 404 },
        async () => {
    ...
    await harness.run(
        'Not-found access-code reads return 404',
        'GET /section/999999/access-codes',
        { method: 'GET', client: 'professor' },
        { status: 404 },
        async () => {
            const response = await professor.request('/section/999999/access-codes');
