...
export async function runCatalogSuite(env) {
    ...
    await harness.run(
        'Course creation validates request bodies',
        'POST /course',
        { method: 'POST', client: 'admin', body: { code: 'CMSC900', title: 'Broken' } },
        { status: 400 },
        async () => {
    ...
    await harness.run(
        ...
        async () => {
            ...
            assert(response.body.Course.some((entry) => entry.Course.course_id === ctx.course1.course_id), 'Course list is missing course1.');
            return harness.responseSummary(response);
        }
    );

    await harness.run(
        'Public course list rejects invalid subject filters',
        'GET /course?subject=ZZZZ',
        { method: 'GET', query: { subject: 'ZZZZ' } },
        { status: 400 },
        async () => {
    ...
    await harness.run(
        'Public course reads return a single course',
        `GET /course/${ctx.course1?.course_id ?? ':id'}`,
        { method: 'GET' },
        { status: 200 },
        async () => {
    ...
    await harness.run(
        'Public prerequisite reads return wrapped prerequisite items',
        `GET /prerequisite/${ctx.course1?.course_id ?? ':id'}`,
        { method: 'GET' },
        { status: 200 },
        async () => {
            ...
            assert(response.body.some((entry) => entry.Prerequisite.courseId === ctx.course2.course_id), 'Prerequisite list is missing course2.');
            return harness.responseSummary(response);
        }
    );

    await harness.run(
        'Semester creation validates request bodies',
        'POST /semester',
        { method: 'POST', client: 'admin', body: { term: 'Fall' } },
        { status: 400 },
        async () => {
    ...
    await harness.run(
        ...
        'POST /semester',
        { method: 'POST', client: 'admin', body: { term: 'Winter', year: 2030 } },
        { status: 201 },
        async () => {
            const response = await admin.request('/semester', {
                method: 'POST',
    ...
    await harness.run(
        'Semester creation rejects duplicates',
        'POST /semester',
        { method: 'POST', client: 'admin', body: { term: 'Winter', year: 2030 } },
        { status: 409 },
        async () => {
    ...
    await harness.run(
        'Public semester list returns array-wrapped semesters',
        'GET /semester',
        { method: 'GET' },
        { status: 200 },
        async () => {
            ...
            assert(response.body.some((entry) => entry.Semester.semester_id === ctx.semester.semester_id), 'Semester list is missing the created semester.');
            return harness.responseSummary(response);
        }
    );

    await harness.run(
        'Public semester reads return a single semester',
        `GET /semester/${ctx.semester?.semester_id ?? ':id'}`,
        { method: 'GET' },
        { status: 200 },
        async () => {
    ...
    await harness.run(
        'Public section reads return a single section',
    ...
    await harness.run(
        ...
        async () => {
            ...
            assert(response.body.Section.some((entry) => entry.Section.section_id === ctx.section.section_id), 'Section list is missing the created section.');
            return harness.responseSummary(response);
        }
    );
    ...
    await harness.run(
        ...
        async () => {
            ...
            assert(responses.every((response) => response.status === 200), 'At least one concurrent section read failed.');
