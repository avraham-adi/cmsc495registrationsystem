...
export async function runSessionAuthSuite(env) {
	...
	await harness.run('First-login students can still fetch /user/me', 'GET /user/me', { method: 'GET', client: 'student' }, { status: 200 }, async () => {
	...
	await harness.run(
		...
		async () => {
			...
			assert(response.body?.error === 'Password change required before accessing this resource.', 'Expected first-login block for student.');
			return harness.responseSummary(response);
		}
	);

	await harness.run('Password updates enforce validation rules', 'PATCH /user/me', { method: 'PATCH', client: 'student', body: { password: 'short' } }, { status: 400 }, async () => {
		const response = await student.request('/user/me', {
			method: 'PATCH',
			body: { password: 'short' },
		});
	...
	await harness.run('Password updates reject the literal default password', 'PATCH /user/me', { method: 'PATCH', client: 'student', body: { password: 'Password' } }, { status: 400 }, async () => {
		const response = await student.request('/user/me', {
			method: 'PATCH',
	...
	await harness.run('Password change invalidates the prior session version', 'GET /user/me', { method: 'GET', client: 'staleStudent' }, { status: 401 }, async () => {
	...
	await harness.run(
		...
		async () => {
			...
			assert(
				...
				'Every stale session request should be rejected after password rotation.'
			);
			return responses.map(harness.responseSummary);
		}
	);

	await harness.run('Current student session remains valid after password rotation', 'GET /user/me', { method: 'GET', client: 'student' }, { status: 200 }, async () => {
		const response = await student.request('/user/me');
		assertStatus(response, 200);
	...
	await harness.run('A second active session is invalidated after password rotation', 'GET /user/me', { method: 'GET', client: 'otherStudent-secondary' }, { status: 401 }, async () => {
	...
	await harness.run('Non-admin authenticated users cannot access admin routes', 'GET /admin', { method: 'GET', client: 'student' }, { status: 403 }, async () => {
