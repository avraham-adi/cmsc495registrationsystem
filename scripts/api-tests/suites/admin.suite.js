...
export async function runAdminSuite(env) {
	...
	await harness.run('Admin user creation validates request bodies', 'POST /admin', { method: 'POST', client: 'admin', body: { name: 'Broken User' } }, { status: 400 }, async () => {
	...
	await harness.run('Admin can create a professor user', 'POST /admin', { method: 'POST', client: 'admin', body: ctx.users.professor }, { status: 201, userType: 'PROFESSOR' }, async () => {
		const response = await admin.request('/admin', {
			method: 'POST',
			body: {
	...
	await harness.run('Admin can create a student user', 'POST /admin', { method: 'POST', client: 'admin', body: ctx.users.student }, { status: 201, userType: 'STUDENT' }, async () => {
		const response = await admin.request('/admin', {
			method: 'POST',
			body: {
	...
	await harness.run('Admin can create a second student user', 'POST /admin', { method: 'POST', client: 'admin', body: ctx.users.otherStudent }, { status: 201, userType: 'STUDENT' }, async () => {
		const response = await admin.request('/admin', {
			method: 'POST',
			body: {
	...
	await harness.run('Admin can create a temporary user for role-regression checks', 'POST /admin', { method: 'POST', client: 'admin', body: ctx.users.temp }, { status: 201 }, async () => {
		const response = await admin.request('/admin', {
			method: 'POST',
			body: {
	...
	await harness.run('Admin user creation rejects duplicate emails', 'POST /admin', { method: 'POST', client: 'admin', body: ctx.users.student }, { status: 409 }, async () => {
		const response = await admin.request('/admin', {
			method: 'POST',
			body: {
				name: ctx.users.student.name,
	...
	await harness.run('Admin can fetch a created user by id', `GET /admin/${ctx.users.professor.user?.id ?? ':id'}`, { method: 'GET', client: 'admin' }, { status: 200 }, async () => {
